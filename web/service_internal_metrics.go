...
var internalGaugeStats = []string{
	...
	"user.threads", "user.nroc.hard", "user.nproc.soft",
	...
	return []domain.GraphConfig{
		{
			...
			// SRE-42 NPROC limits and users threads
			ID:          "userThreads",
			Name:        "Threads/NPROC limits of zenoss user",
			BuiltIn:     true,
			Format:      "%4.2f",
			ReturnSet:   "EXACT",
			Type:        "line",
			Tags:        tags,
			YAxisLabel:  "threads",
			Description: "amount of threads/NPROC limits of zenoss user",
			MinY:        &zero,
			Range:       &tRange,
			Units:       "threads",
			DataPoints: []domain.DataPoint{
