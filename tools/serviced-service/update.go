...
type Update struct {
	Args struct {
		TemplateFile flags.Filename `positional-arg-name:"TEMPLATE_FILE" description:"Service template file (to get log filters)"`
		ServicesFile flags.Filename `positional-arg-name:"SERVICES_FILE" description:"Service Definition file"`
...
func (c *Update) Execute(args []string) error {
	...
	var migrated = migrator.Migrate(updates)

	marshaled, err := json.MarshalIndent(migrated, "", "  ")
	if err != nil {
...
func updateServices(services *ServiceList, updates dao.ServiceMigrationRequest) {
}

func update(services *ServiceList, tenantID string, svc *service.Service) error {
	// cursvc, err := f.validateUpdate(&svc)
	// if err != nil {
	// 	return err
	// }
	current, err := services.Get(svc.ID)
	if err != nil {
		return err
	}
	if current == nil {
		return fmt.Errorf("No service to update  name=%s id=%s", svc.Name, svc.ID)
	}
